...
三步学习流程：**Codex 生成阅读文章 → 用户用遗忘词造句 → Codex 批改 → 页面展示批改结果**。用故事记忆法替代死记硬背，文章做 SIELE B1+-B2 水平。
...
**初始化文本格式**（Codex 生成，用户粘贴到 `vocabInitInput`）：
```
---practice---
title: 练习标题
...
**造句暂存**：用户在 `practiceSentencesInput` 中写 `word: sentence` 格式，点「💾 暂存」→ 自动复制到剪贴板（`navigator.clipboard.writeText`），发给 Codex 批改。
...
- **遗忘词造句**：一次性批量写 + 批量批改。批改格式：`---单词--- 原句/改句/说明 ---`。
  **批改原则**：只改确实有错的地方。不确定的搭配（如动词+介词、特定表达）先查清楚再出手，不要凭"语感"乱改。用户自己写对的规则（如原形名词化、固定搭配）不要硬纠正。之前改错过 *desprenderse de trabajar*（语法对但被改成名词）、*padecer de*（正确搭配被改成 por），这种错误让用户更混乱。不确定的时候宁可不改，只指出"可能不常见但能懂"。
...
- **每日报告**：点📋日报告会汇总打卡+笔记+变位错题+遗忘词练进度。用户粘贴日报后 Codex 生成总结，用户再粘贴到 📊 学习汇总。
